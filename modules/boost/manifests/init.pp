...
# TODO: Get it to determine the compiler version from the compiler, so it doesn't have to be manually bumped

define boost ( $boost_version, $cpp_standard, $set_opt_include_symlink = false ) {
	$user            = 'lewis'
	$group           = 'lewis'
	$home_dir        = "/home/${user}"
	...
	$cpp_standard_year = $cpp_standard ? {
		/^c\+\+(\d+)$/ => Integer( $1 ),
		# Correctly fails compilation if no match is found and there isn't a default
	}
	$standard_specific_cxx_flags = ( $cpp_standard_year >= 17 ) ? {
		true  => ' -DBOOST_ASIO_HAS_STD_STRING_VIEW ',
		false => '',
	}

	$tag_compiler_and_cmake_flags_sets = [
		[  '9.0', 'normal', 'clang', '/usr/bin/clang++',                    "                         cxxflags=\"-std=${cpp_standard} -stdlib=libc++                                                                                                                                                                                                       ${standard_specific_cxx_flags} \" linkflags=\"-std=${cpp_standard} -stdlib=libc++                                                                                                                                                                                                       -DBOOST_ASIO_HAS_STD_STRING_VIEW \" " ],
		[ '10.0', 'dbgchk', 'clang', "${home_dir}/source/llvm/bin/clang++", " define=_LIBCPP_DEBUG=0  cxxflags=\"-std=${cpp_standard} -stdlib=libc++                                                                                                                                                                                                       ${standard_specific_cxx_flags} \" linkflags=\"-std=${cpp_standard} -stdlib=libc++                                                                                                                                                                                                       -DBOOST_ASIO_HAS_STD_STRING_VIEW \" " ],
		[ '10.0', 'ubasan', 'clang', "${home_dir}/source/llvm/bin/clang++", "                         cxxflags=\"-std=${cpp_standard} -stdlib=libc++ -fsanitize=address -fsanitize=undefined -fno-omit-frame-pointer                                                                                                                                       ${standard_specific_cxx_flags} \" linkflags=\"-std=${cpp_standard} -stdlib=libc++ -fsanitize=address -fsanitize=undefined -fno-omit-frame-pointer                                                                                                                                       -DBOOST_ASIO_HAS_STD_STRING_VIEW \" " ],
		[ '10.0', 'thrsan', 'clang', "${home_dir}/source/llvm/bin/clang++", "                         cxxflags=\"-std=${cpp_standard} -stdlib=libc++ -fsanitize=thread                                                                                                                                                                                     ${standard_specific_cxx_flags} \" linkflags=\"-std=${cpp_standard} -stdlib=libc++ -fsanitize=thread                                                                                                                                                                                     -DBOOST_ASIO_HAS_STD_STRING_VIEW \" " ],
		[ '10.0', 'memsan', 'clang', "${home_dir}/source/llvm/bin/clang++", "                         cxxflags=\"-std=${cpp_standard} -stdlib=libc++ -fsanitize=memory -fsanitize-memory-track-origins -fPIE -fPIC -pie -fno-omit-frame-pointer -nostdinc++ -I${msan_libcxx_dir}/include/c++/v1 -L${msan_libcxx_dir}/lib -Wl,-rpath,${msan_libcxx_dir}/lib ${standard_specific_cxx_flags} \" linkflags=\"-std=${cpp_standard} -stdlib=libc++ -fsanitize=memory -fsanitize-memory-track-origins -fPIE -fPIC -pie -fno-omit-frame-pointer -nostdinc++ -I${msan_libcxx_dir}/include/c++/v1 -L${msan_libcxx_dir}/lib -Wl,-rpath,${msan_libcxx_dir}/lib -DBOOST_ASIO_HAS_STD_STRING_VIEW \" " ],
		[ '10.2', 'normal', 'gcc',   "${home_dir}/source/gcc/bin/g++",      "                         cxxflags=\"-std=${cpp_standard}                                                                                                                                                                                                                      ${standard_specific_cxx_flags} \" linkflags=\"-std=${cpp_standard}                                                                                                                                                                                                                      -DBOOST_ASIO_HAS_STD_STRING_VIEW \" " ],
		[ '10.2', 'dbgchk', 'gcc',   "${home_dir}/source/gcc/bin/g++",      " define=_GLIBCXX_DEBUG=1 cxxflags=\"-std=${cpp_standard}                                                                                                                                                                                                                      ${standard_specific_cxx_flags} \" linkflags=\"-std=${cpp_standard}                                                                                                                                                                                                                      -DBOOST_ASIO_HAS_STD_STRING_VIEW \" " ],
		[ '10.2', 'ubasan', 'gcc',   "${home_dir}/source/gcc/bin/g++",      "                         cxxflags=\"-std=${cpp_standard}                -fsanitize=address -fsanitize=undefined -fno-omit-frame-pointer                                                                                                                                       ${standard_specific_cxx_flags} \" linkflags=\"-std=${cpp_standard}                -fsanitize=address -fsanitize=undefined -fno-omit-frame-pointer                                                                                                                                       -DBOOST_ASIO_HAS_STD_STRING_VIEW \" " ],
	]
   ...
   # # TEMPORARILY RESTRICT CLANG TO 7.0 FOR BOOST 1.68 #
	...
	# 	# [ '7.0', 'dbgchk', 'clang', '/usr/bin/clang++',                    " define=_LIBCPP_DEBUG=0  cxxflags=\"-std=${cpp_standard} -stdlib=libc++                                                                                                                                                                                                       -DBOOST_ASIO_HAS_STD_STRING_VIEW \" linkflags=\"-std=${cpp_standard} -stdlib=libc++                                                                                                                                                                                                       -DBOOST_ASIO_HAS_STD_STRING_VIEW \" " ],
	...
	# 	# [ '9.1', 'dbgchk', 'gcc',   "${home_dir}/source/gcc/bin/g++",      " define=_GLIBCXX_DEBUG=1 cxxflags=\"-std=${cpp_standard}                                                                                                                                                                                                                      -DBOOST_ASIO_HAS_STD_STRING_VIEW \" linkflags=\"-std=${cpp_standard}                                                                                                                                                                                                                      -DBOOST_ASIO_HAS_STD_STRING_VIEW \" " ],
	...
	$tag_compiler_and_cmake_flags_sets.each | Array $tag_compiler_and_cmake_flags_set | {
		...
		$compiler_major_version          = split( $compiler_version, /\./ )[ 0 ]
		...
		if $b2_toolset == 'clang' and $build_type_tag == 'normal' and $set_opt_include_symlink {
